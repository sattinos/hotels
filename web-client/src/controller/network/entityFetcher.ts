import { SortItem } from './../lib/queryLanguage';
...
export class EntityFetcher<Entity> {
    ...
    public async getAll(pageIndex: number = 0, pageSize: number = 10, sortItems?: SortItem[], isLog?: boolean): Promise<Entity[]> {
        try {
            const url = `${this.baseUrl}${this.path}/${pageIndex}/${pageSize}`;
            const params = sortItems ? {
                sortItems
            } : undefined;
            const response = await axios({
                method: 'get',
    ...
    public async deleteMany(ids: number[]) {
        try {
            ...
            // console.log(url);            console.log(ids);
            const response = await axios({
                method: 'delete',
                url,
                data: ids
